...
$$M[X] = 0 + \int _{ a } ^{ b } x f(x) dx + 0$$

$$M[X] = \int _{ 1 } ^{ 2 } 3x (x-1)^2 dx$$

$$M[X] = \int _{ 1 } ^{ 2 } 3x (x-1)^2 dx$$

Ищем неопределенный интрегал:
...
$$3 (\frac{ x^4 }{ 4 } -2 \frac{ x^3 }{ 3 } + \frac{ x^2 }{ 2 } + E)$$

$$\frac{ 3 }{ 4 } x^4 -2 x^3 + \frac{ 3 }{ 2 } x^2 + 3E$$

Подставляем пределы:

$$(\frac{ 3 }{ 4 } x^4 -2 x^3 + \frac{ 3 }{ 2 } x^2) | _1 ^2$$

$$\frac{ 3 }{ 4 } (x^4|_1^2) -2 (x^3|_1^2) + \frac{ 3 }{ 2 } (x^2|_1^2)$$

$$\frac{ 3 * 15 }{ 4 } -2 * 7 + \frac{ 3 * 3 }{ 2 }$$

$$\frac{ 45 }{ 4 } - 14 + \frac{ 9 }{ 2 }$$

$$M[X] = 11.25 - 14 + 4.5 = \boxed{1.75}$$
...
$$M[X^2] = 0 + \int _{ a } ^{ b } x^2 f(x) dx + 0$$

$$M[X^2] = \int _{ 1 } ^{ 2 } x^2 \cdot 3(x-1)^2 dx$$
...
$$3 \int x^2 (x^2 - 2x + 1) dx$$

$$3 \int (x^4 - 2x^3 + x^2) dx$$

$$3 (\frac{ x^5 }{ 5 } -2 \frac{ x^4 }{ 4 } + \frac{ x^3 }{ 3 } + E)$$

$$\frac{ 3 }{ 5 } x^5 -\frac{ 3 }{ 2 } x^4 + x^3 + 3E$$

Подставляем пределы:

$$(\frac{ 3 }{ 5 } x^5 -\frac{ 3 }{ 2 } x^4 + x^3) \big| _1 ^2$$

$$\frac{ 3 }{ 5 } (x^5|_1^2) -\frac{ 3 }{ 2 } (x^4|_1^2) + (x^3|_1^2)$$

$$\frac{ 3 }{ 5 } (32-1) -\frac{ 3 }{ 2 } (16-1) + (8-1)$$

$$\frac{ 3 * 31 }{ 5 } -\frac{ 3 * 15 }{ 2 } + 7$$

$$\frac{ 93 }{ 5 } -\frac{ 45 }{ 2 } + 7$$

$$M[X^2] = 18.6 - 22.5 + 7 = 25.6 - 22.5 = 3.1$$

$$D[X] = 3.1 - 1.75^2 = 3.1 - 3.0625 = \boxed{0.0375}$$
...
```math
\sqrt{0.0375}
= \sqrt{375 \cdot 0.0001}
= \frac{ 1 }{ 100 } \sqrt{375}
= \frac{ 5 }{ 100 } \sqrt{15}
= \frac{ \sqrt{15} }{ 20 }
```

$$\boxed{\sigma[X] = \frac{ \sqrt{15} }{ 20 } \approx 0.1936491673}$$
...
### используя дифференциальную функцию (плотности)

$$P(X > 1.5) = \int _{ 1.5 } ^{ +\infty } f(x) dx$$

```math
P(X > 1.5) = \int _{ 1.5 } ^{ 2 } 3(x-1)^2 dx + \int _{ 2 } ^{ +\infty } 0 dx
```

$$\int 3(x-1)^2 dx = x^3 - 3x^2 + 3x + 3D$$

(где $D$ - константа)

```math
P(X > 1.5) = \left. (x^3 - 3x^2 + 3x) \right| _{1.5} ^2 + 0
```

```math
P(X > 1.5) = (2^3 - 1.5^3) - 3(2^2 - 1.5^2) + 3(2 - 1.5)
```

```math
P(X > 1.5) = (8 - 3.375) - 3(4 - 2.25) + 3 \cdot 0.5
```

```math
P(X > 1.5) = 4.625 - 3 \cdot 1.75 + 1.5
```

```math
\boxed {
    P(X > 1.5) = 6.125 - 5.25 = 0.875
}
```
...
> ![alt text](normal-3.png)
>
> $$S_\text{выделенной фигуры} = 0.8386$$

$$\gamma = 0.8386$$

иными словами найти такой $(a-\lambda; a+\lambda)$, при котором
...
## 4. интревал, в котором окажутся практически все значения X

> ![alt text](normal-4.png)
>
> все три сигмы в действии. На третьей даже не видно границы покрытия
...
$$P(|X-m| < \sigma t) = 2\Phi _0(t)$$

при $t=3$:

$$P(|X-m| < 3\sigma) = 2\Phi _0(3) \approx 0.9973$$

$$0.9973 \approx P(a - 3\sigma < X < a + 3\sigma)$$
